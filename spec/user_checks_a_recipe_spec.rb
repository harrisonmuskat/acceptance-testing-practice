...
feature "User checks a recipe's deliciousness", %(
  ...
  scenario "user visits the root path and sees a form" do
    visit '/'
  ...
  scenario "user submits a recipe name containing 'pickled beets'" do
    visit '/'
    fill_in 'Recipe Name', with: 'Quick Pickled Beets'
    click_button "Submit"

    expect(page).to have_current_path('/')
    expect(page).to have_content('"Quick Pickled Beets" is a delicious recipe!')
  end

  scenario "user submits a recipe name without 'pickled beets'" do
    visit '/'
    fill_in 'Recipe Name', with: 'Quick Pickled Bananas'
    click_button "Submit"

    expect(page).to have_current_path('/')
    expect(page).to have_content('"Quick Pickled Bananas" is not a delicious recipe!')
  end
